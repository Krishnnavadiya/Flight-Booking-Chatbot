...
class FlightBot extends ActivityHandler {
    constructor(conversationState, userState, luisRecognizer) {
        ...
        this.onMessage(async (context, next) => {
            ...
            // First, we use the dispatch model to determine which cognitive service to use.
            ...
            switch (intent) {
                case 'BookFlight':
                    await this.flightBookingDialog.run(context, this.dialogState, recognizerResult);
                    break;
                case 'SearchFlights':
                    await this.flightSearchDialog.run(context, this.dialogState, recognizerResult);
                    break;
                case 'CompareFlights':
                    await this.flightComparisonDialog.run(context, this.dialogState, recognizerResult);
                    break;
                case 'BookTicket':
                    await this.bookingDialog.run(context, this.dialogState, recognizerResult);
                    break;
                case 'Cancel':
